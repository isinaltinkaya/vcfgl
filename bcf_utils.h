...
typedef struct simRecord {
    ...
    // either the record copied from input file or blank record created by
    // -explode 1
    bcf1_t* rec = NULL;

    // acgt_n_q13_bases	number of bases where quality score >= 13 summed across
    // all individuals
    //
    ...
    //
    // e.g.
    // 		acgt_n_q13_bases[2] == #C bases that are forward and have
    // quality score >= 13
    ...
    // sim->set_tag_I16(&acgt_n_q13_bases[0][0], &acgt_sum_taildist[0],
    // &acgt_sum_taildist_sq[0], &acgt_sum_qs[0], &acgt_sum_qs_sq[0]);
    void set_tag_I16();
