...
simRecord::simRecord(bcf_hdr_t* in_hdr) {
    ...
    // max
    this->max_size_bcf_tag_number =
        (int*)malloc(N_ENUM_BCF_TAG_NUMBER * sizeof(int));
    this->current_size_bcf_tag_number =
        (int*)malloc(N_ENUM_BCF_TAG_NUMBER * sizeof(int));

    this->nHaplotypes = this->nSamples * SIM_PLOIDY;
    this->max_size_bcf_tag_number[FMT_NUMBER_GT] = this->nHaplotypes;
    this->current_size_bcf_tag_number[FMT_NUMBER_GT] =
        this->max_size_bcf_tag_number[FMT_NUMBER_GT];

    this->max_size_bcf_tag_number[FMT_NUMBER_1] = this->nSamples;
    // fixed size for all sites
    this->current_size_bcf_tag_number[FMT_NUMBER_1] =
        this->max_size_bcf_tag_number[FMT_NUMBER_1];
    ...
    this->current_size_bcf_tag_number[FMT_NUMBER_G] =
        this->max_size_bcf_tag_number[FMT_NUMBER_G];
    ...
    this->current_size_bcf_tag_number[FMT_NUMBER_R] =
        this->max_size_bcf_tag_number[FMT_NUMBER_R];
    ...
    this->current_size_bcf_tag_number[INFO_NUMBER_G] =
        this->max_size_bcf_tag_number[INFO_NUMBER_G];

    this->max_size_bcf_tag_number[INFO_NUMBER_R] = MAX_NALLELES;
    // variable size depending on the number of possible alleles at each site
    this->current_size_bcf_tag_number[INFO_NUMBER_R] =
        this->max_size_bcf_tag_number[INFO_NUMBER_R];
        ...
        if (args->verbose > 0) {
            for (int sample_i = 0; sample_i < this->nSamples; sample_i++) {
                fprintf(stderr,
                        ...
                        "set to %f\n",
                        bcf_hdr_int2id(this->hdr, BCF_DT_SAMPLE, sample_i),
...
void simRecord::add_tags() {
    if (args->addFormatDP) {
        ASSERT(0 ==
               (bcf_update_format_int32(
                   this->hdr, this->rec, "DP", this->fmt_dp_arr,
                   this->current_size_bcf_tag_number[bcf_tags[FMT_DP].n])));
    }
    ...
    if (args->addGL) {
        ASSERT(0 == (bcf_update_format_float(
                        this->hdr, this->rec, "GL", this->gl_arr,
                        this->current_size_bcf_tag_number[bcf_tags[GL].n])));
    }
    ...
    if (args->addFormatAD) {
        ASSERT(0 ==
               (bcf_update_format_int32(
                   this->hdr, this->rec, "AD", this->fmt_ad_arr,
                   this->current_size_bcf_tag_number[bcf_tags[FMT_AD].n])));
    }
    if (args->addFormatADF) {
        NEVER;  // TODO
        ASSERT(0 ==
...
void simRecord::add_tags_missing() {
    ...
    if (args->addGL) {
        bcf_tag_reset_max<float>(this->gl_arr, GL, bcf_float_missing_union_f,
                                 this);
        ASSERT(0 == (bcf_update_format_float(
                        this->hdr, this->rec, "GL", this->gl_arr,
                        this->current_size_bcf_tag_number[bcf_tags[GL].n])));
    ...
    if (args->addGP) {
        bcf_tag_reset_max<float>(this->gp_arr, GP, bcf_float_missing_union_f,
                                 this);
        ASSERT(0 == (bcf_update_format_float(
                        this->hdr, this->rec, "GP", this->gp_arr,
                        this->current_size_bcf_tag_number[bcf_tags[GP].n])));
    ...
    if (args->addQS) {
        bcf_tag_reset_max<float>(this->qs_arr, QS, bcf_float_missing_union_f,
                                 this);
        ASSERT(0 == (bcf_update_info_float(
                        this->hdr, this->rec, "QS", this->qs_arr,
                        this->current_size_bcf_tag_number[bcf_tags[QS].n])));
    ...
    if (args->addI16) {
        // this->i16_arr = bcf_tag_alloc_max<float>(I16,
        // bcf_float_missing_union_f, this);
        ASSERT(0 == (bcf_update_info_float(this->hdr, this->rec, "I16",
                                           this->i16_arr, 16)));
    }
...
void simRecord::set_acgt_alleles_luts(int* acgt_arr) {
    // DEVPRINT("%c:%d %c:%d %c:%d
    // %c:%d","ACGT"[0],acgt_arr[0],"ACGT"[1],acgt_arr[1],"ACGT"[2],acgt_arr[2],"ACGT"[3],acgt_arr[3]);
    ...
    // DEVPRINT("-> ordered %c:%d %c:%d %c:%d
    // %c:%d","ACGT"[alleles2acgt[0]],acgt_arr[alleles2acgt[0]],"ACGT"[alleles2acgt[1]],acgt_arr[alleles2acgt[1]],"ACGT"[alleles2acgt[2]],acgt_arr[alleles2acgt[2]],"ACGT"[alleles2acgt[3]],acgt_arr[alleles2acgt[3]]);
    // DEVPRINT("-> acgt %c:%d %c:%d %c:%d
    // %c:%d","ACGT"[0],acgt_arr[0],"ACGT"[1],acgt_arr[1],"ACGT"[2],acgt_arr[2],"ACGT"[3],acgt_arr[3]);
...
void simRecord::set_tag_I16(void) {
    if (1 == args->addI16) {
        ...
        this->i16_arr[0] =
            this->acgt_n_q13_bases[(refb * 2) + SIM_FORWARD_STRAND];

        // 2   #reference Q13 bases on the reverse strand
        this->i16_arr[1] =
            this->acgt_n_q13_bases[(refb * 2) + SIM_REVERSE_STRAND];
        ...
        for (int a = 1; a < this->nAlleles; ++a) {
            ...
            this->i16_arr[2] +=
                this->acgt_n_q13_bases[(b * 2) + SIM_FORWARD_STRAND];

            // 4   #non-ref Q13 bases on the reverse strand
            this->i16_arr[3] +=
                this->acgt_n_q13_bases[(b * 2) + SIM_REVERSE_STRAND];
